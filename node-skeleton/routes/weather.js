...
module.exports = (db) => {
  function weatherFormater(latitude, longitude) {
    ...
    axios.get(url).then((response) => {
      ...
      for (const item of weatherData.daily) {
        ...
        if (!formatingData["days"]["current_day"]) {
          ...
          formatingData["days"]["current_day"] = {
            ...
            sunrise: sunRise.toLocaleDateString("en-CA"),
            sunset: sunSet.toLocaleDateString("en-CA"),
      ...
      console.log("inside",formatingData)
  ...
  router.get("/", (req, res) => {
    ...
    console.log("HERE:", weatherFormater(51.0447, -114.0719));
    //   .then((res) => {
    //     res.send(res);
    //   })
    //   .catch((err) => {
